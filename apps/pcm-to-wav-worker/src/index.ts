...
function trimSilence(pcmData: Uint8Array, threshold: number = 10): Uint8Array {
    let start = 0;
    for (let i = 0; i < pcmData.length - 1; i += 2) {
        const sample = new Int16Array(pcmData.buffer, i, 1)[0];
        if (Math.abs(sample) > threshold) { // Use a small threshold instead of strict 0
            start = Math.max(0, i - 20); // Back up slightly to preserve onset (e.g., 10ms at 48kHz)
            break;
        }
    }
    if (start >= pcmData.length) return new Uint8Array(0);
    return pcmData.slice(start);
}
...
export default class PcmToWavWorker extends WorkerEntrypoint<Env> {
	async fetch(request: Request) {
		...
		if (url.pathname.startsWith('/convert/')) {
			...
			const { startChunk, endChunk } = await request.json<{ roomId: string; startChunk: number; endChunk: number }>();
        ...
        const pcmKey = `${roomId}/pcm/${i}.pcm`;
			...
			const trimmedPcm = trimSilence(combinedPcm);
			if (trimmedPcm.length === 0) {
				console.log('[PcmToWavWorker] No audio after trimming silence');
        const wavKey = `${roomId}/wav/${startChunk}-${endChunk}.wav`;
			...
			const wavHeader = createWavHeader(trimmedPcm.length);
			const fullWav = new Uint8Array([...wavHeader, ...trimmedPcm]);
