      ...
      <div className="__dropdown-button w-56 relative">
        ...
        <Listbox value={selectedUserGroup} onChange={(value) => setModeView(value.name)}>
          <div className="mt-1 relative">
            <ListboxButton className="relative w-full bg-white border border-gray-300 rounded-md shadow-2xs pl-3 pr-10 py-2 text-center cursor-default focus:outline-hidden focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
              <span className="block truncate">{`${modeView} ${groupEmojis[modeView]}`}</span>
              <span className="absolute inset-y-0 right-0 flex items-center pr-2 pointer-events-none">
                <ChevronUpDownIcon className="h-5 w-5 text-gray-400" aria-hidden="true" />
              </span>
            </ListboxButton>
            <ListboxOptions className="absolute z-10 mt-1 w-full bg-white shadow-lg max-h-60 rounded-md py-1 text-base ring-1 ring-black ring-opacity-5 overflow-auto focus:outline-hidden sm:text-sm"
            >
