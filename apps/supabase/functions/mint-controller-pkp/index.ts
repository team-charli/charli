...
// Must match your Supabase Edge Function name
const functionName = 'mint-controller-pkp'
const app = new Hono().basePath(`/${functionName}`)

/** --- SET UP CORS MIDDLEWARE --- **/
// This automatically handles OPTIONS requests
app.use(
  '*',
  cors({
    origin: '*',
...
/**
 * Main route: POST /
 * Supabase will call it at:
 *   POST /functions/v1/mint-controller-pkp
 */
app.post('/', async (c) => {
  try {
...
async function mintAndBurnPKP(
  sessionId: number,
  litNodeClient: LitNodeClient.LitNodeClientNodeJs,
  wallet: ethers.Wallet,
  pkpNftContractAddress: ethers.AddressLike
) {
  try {
    console.log('sessionId', sessionId)
    const { data, error } = await supabaseClient.from('sessions').select('key_claim_data').eq('session_id', sessionId)
    ...
    const contractClient = new LitContracts({
      ...
      provider,
      debug: false,
      options: { storeOrUseStorageKey: false },
    })
    ...
    const abi = [
      ...
      },
      {
        inputs: [
          ...
          { internalType: 'address', name: 'stakingContractAddress', type: 'address' },
        ],
        name: 'claimAndMint',
        outputs: [{ internalType: 'uint256', name: '', type: 'uint256' }],
    ...
    const formattedDerivedKeyId = ethers.utils.hexlify(ethers.utils.hexZeroPad(`0x${derivedKeyId}`, 32))
    const claimAndMintTx = await pkpNftContract.claimAndMint(2, formattedDerivedKeyId, signatures, stakingContractAddress, {
      value: pkpMintCost,
    })
...
/**
 * Error handling, if you want a custom fallback
 * (Optional - Hono has some built-in default handling)
 */
app.onError((err, c) => {
