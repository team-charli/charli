...
export const litLessFetchJwt = async () => {
  // Fetch new nonce
  ...
  let signature = await litLessWallet.signMessage(nonceResponse.nonce).catch(e => console.error("problem signing nonce", e));

  console.log('run jwt request');
  let jwtResponse = await ky.post('https://supabase-auth.zach-greco.workers.dev/jwt', {
    json: { ethereumAddress: litLessWallet.address, signature, nonce: nonceResponse.nonce },
  })
  .json<{ token: string }>()
  .catch(e => {
