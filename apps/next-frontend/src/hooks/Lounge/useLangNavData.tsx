...
export const useLangNavData = (modeView: 'Learn' | 'Teach' | 'Schedule') => {
  ...
  const { data: languageData, isLoading, error } = useQuery({
    queryKey: ['langNavData', isOnboarded, supabaseClient],
  ...
  useEffect(() => {
    if (languagesToShow.length > 0 && !languagesToShow.includes(selectedLang)) {
      setSelectedLang(languagesToShow[0]);
    }
